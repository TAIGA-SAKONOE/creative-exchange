          ...
          <div className="p-10">
            ...
            {personalPrices.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                まだ取引実績がありません<br />取引が蓄積されるとここに価格帯が表示されます
              </div>
            ) : (
                  ...
                  <div key={item.category_id} className="bg-gray-50 border border-gray-200 rounded-2xl p-6">
                    ...
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        ...
                        <span className="font-bold text-xl">¥{Math.round(item.median).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">価格帯（25〜75パーセンタイル）</span>
                        <span>¥{Math.round(item.p25).toLocaleString()} 〜 ¥{Math.round(item.p75).toLocaleString()}</span>
                      </div>
                      <div className="text-xs text-gray-500">
                        取引件数: {item.count}件（直近180日）
                      </div>
                    </div>
                  </div>
                ))}
